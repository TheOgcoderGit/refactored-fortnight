...
async def _get_chat(client, username, for_destination=False):
    ...
    try:
        ...
        logger.warning("get_chat failed for %r: %s", username, e)
        # If user entered a numeric ID, provide fallback rather than hard crashing
        ...
        raise RuntimeError(f"Could not find or join chat ({e}). Ensure your connected Telegram account has access.")
...
@asynccontextmanager
async def _owner_client(user_id):
    """
    ...
    - Falls back to shared client for administrators
    """
    ...
    if not row:
        if user_id in ADMIN_IDS:
            ...
            return
        raise RuntimeError("Please connect your Telegram account with /connect first.")
    ...
    try:
        ...
        if not await client.is_user_authorized():
            raise RuntimeError("Telegram session expired. Please /connect again.")
        yield client
    finally:
